document.addEventListener('DOMContentLoaded', async () => {
    ...
    function calculateAttendanceStatus(registeredFace, now) {
        ...
        const departureTimeStr = registeredFace?.departureTime || defaultDepartureTime;
    ...
    async function startCamera() {
        try {
            ...
            video.addEventListener('error', (err) => {
                ...
                statusMessage.textContent = "Kesalahan kamera. Pastikan kamera tidak digunakan aplikasi lain.";
            ...
            statusMessage.style.color = 'red';
            statusMessage.textContent = "Gagal mengakses kamera. Periksa izin atau hardware.";
        }
    }
    ...
    async function scanFace() {
        ...
        try {
            ...
            if (bestMatch && highestScore >= SIMILARITY_THRESHOLD) {
                ...
                const attendanceDateDisplay = now.toLocaleDateString('id-ID', {year: 'numeric', month: 'long', day: 'numeric'});
                const { attendanceStatus, displayStatus } = calculateAttendanceStatus(bestMatch, now);
                ...
                if (hasAttendedToday) {
                    ...
                    const latestRecord = attendanceRecords.find(record =>
                        record.idNumber === bestMatch.idNumber && record.date === todayISO
            ...
            if (statusMessage.textContent !== "Kesalahan saat memproses wajah. Coba lagi.") {
                statusMessage.textContent = "Kesalahan saat memproses wajah. Coba lagi.";
                statusMessage.style.color = 'red';
            }
        }
    }
    ...
    function updateDashboardSummary() {
        ...
        if (totalUsersElement && todayAttendanceElement) {
            const registeredFacesCount = JSON.parse(localStorage.getItem('registeredFaces')) || [];
            const attendanceRecordsCount = JSON.parse(localStorage.getItem('attendanceRecords')) || [];

            totalUsersElement.textContent = registeredFacesCount.length;

            const today = new Date().toISOString().slice(0, 10);
            const uniqueAttendanceToday = [...new Set(attendanceRecordsCount
                .filter(record => record.date === today && (record.status === 'Hadir' || record.status === 'Terlambat'))
    ...
    document.getElementById('manualAbsenButton').addEventListener('click', function() {
        ...
        if (hasAttendedToday) {
            ...
            detectedTime.textContent = latestRecord.time; 
            detectedKeterangan.textContent = latestRecord.status;
            if (detectedDate) detectedDate.textContent = now.toLocaleDateString('id-ID', {year: 'numeric', month: 'long', day: 'numeric'});
        ...
        const { attendanceStatus, displayStatus } = calculateAttendanceStatus(registeredFace, now);
        const attendanceTime = now.toLocaleTimeString('id-ID', {hour12: false});
        const attendanceDateDisplay = now.toLocaleDateString('id-ID', {year: 'numeric', month: 'long', day: 'numeric'});
